...
{ 
    ...
    for (int j = 0; j < vertex.size() - 3; j++)
    {
        ...
        for (int i = 1; i <= n; i++)
        {
            drawLine(image, Bezier(vertex[j], vertex[j + 1], vertex[j + 2], vertex[j + 3], static_cast<double>(i - 1) / n), Bezier(vertex[j], vertex[j + 1], vertex[j + 2], vertex[j + 3], static_cast<double>(i) / n), color);
        }   
    }
